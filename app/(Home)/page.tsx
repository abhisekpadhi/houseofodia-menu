...
const Menu: React.FC = () => {
  ...
  useEffect(() => {
    if (params.get("with_location")) {
      setIsWithLocation(params.get("with_location").toLowerCase() === "true");
    }
  }, []);
  ...
  return (
    <div className="bg-black text-white min-h-screen flex px-4 pt-4">
      <div className="max-w-lg w-full">
        ...
        <div className="space-y-8">
          ...
          {Object.keys(menu).map((category) => {
            const filteredItems = menu[category].filter(
              (item) =>
      ...
      {isWithLocation && (
        <button
          onClick={() => {
            window.open("https://maps.app.goo.gl/CioXF1YvSARA8KwP7");
          }}
        ...
        >
          <div className="flex pr-2 flex-col">
            <div className="flex items-center">
              {item.is_veg && <img src="/veg.svg" className="w-6 py-2 mx-0" />}
              {!item.is_veg && (
                <img src="/non_veg.svg" className="w-6 mx-0 py-2" />
              )}
          ...
          <div className="text-yellow-500 font-bold">{item.price}</div>
        </div>
      ))}
    </div>
  );
